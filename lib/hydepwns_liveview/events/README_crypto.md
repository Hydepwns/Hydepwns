...
The `HydepwnsLiveview.Events.CryptoService` module provides secure message encryption and decryption using either Signal Protocol (when available) or a fallback AES-256-CBC implementation. This service is designed to handle secure communication between parties with automatic session management.
...
HydepwnsLiveview.Events.CryptoService.init()
```
...
case HydepwnsLiveview.Events.CryptoService.encrypt_message(reminder, settings) do
...
# Decrypt a message
case HydepwnsLiveview.Events.CryptoService.decrypt_message(encrypted_message, session_id) do
...
{:ok, session_id} = HydepwnsLiveview.Events.CryptoService.create_session("user1", "user2")

# Get session information
{:ok, session} = HydepwnsLiveview.Events.CryptoService.get_session(session_id)

# List all sessions
sessions = HydepwnsLiveview.Events.CryptoService.list_sessions()

# Delete a session
:ok = HydepwnsLiveview.Events.CryptoService.delete_session(session_id)
```
...
{:ok, signature} = HydepwnsLiveview.Events.CryptoService.sign_data(private_key, data)

# Verify signature
{:ok, is_valid} = HydepwnsLiveview.Events.CryptoService.verify_signature(private_key, data, signature)
```
...
{:ok, encrypted} = HydepwnsLiveview.Events.CryptoService.encrypt_with_session(session, "Hello")

# Decrypt with session directly
{:ok, decrypted} = HydepwnsLiveview.Events.CryptoService.decrypt_with_session(session, encrypted)
```
...
stats = HydepwnsLiveview.Events.CryptoService.get_session_stats()
# Returns: %{
#   total_sessions: 5,
#   total_messages: 42,
...
cleaned_count = HydepwnsLiveview.Events.CryptoService.cleanup_expired_sessions()
```
...
mix test test/hydepwns_liveview/events/crypto_service_test.exs

# Run simple crypto tests
mix test test/hydepwns_liveview/events/simple_crypto_test.exs
```
