defmodule HydepwnsLiveview.Events.Core.EventMonitor.Telemetry do
  @moduledoc """
  ...
  def register_handlers do
    config = %{
      enable_external_monitoring: Application.get_env(:hydepwns_liveview, :enable_external_monitoring, false),
      external_monitoring_config: Application.get_env(:hydepwns_liveview, :external_monitoring_config, %{}),
      alert_thresholds: %{
        slow_processing_ms: Application.get_env(:hydepwns_liveview, :slow_processing_threshold_ms, 1000),
        high_queue_size: Application.get_env(:hydepwns_liveview, :high_queue_threshold, 500),
        critical_error_rate: Application.get_env(:hydepwns_liveview, :critical_error_rate_threshold, 0.1)
      },
      enable_detailed_logging: Application.get_env(:hydepwns_liveview, :enable_detailed_telemetry_logging, false)
    }
  ...
  defp handle_telemetry_event([:hydepwns_liveview, :events, :process], measurements, metadata, config) do
    try do
  ...
  defp handle_telemetry_event([:hydepwns_liveview, :events, :queue_size], measurements, metadata, config) do
    try do
  ...
  defp handle_telemetry_event([:hydepwns_liveview, :events, :backpressure], measurements, metadata, config) do
    try do
  ...
  defp handle_telemetry_event([:hydepwns_liveview, :events, :metrics], measurements, metadata, config) do
    try do
  ...
  defp send_metrics_via_http(event_type, metrics_data) do
    try do
      ...
      endpoint = Application.get_env(:hydepwns_liveview, :metrics_http_endpoint)
