defmodule HydepwnsLiveview.Events.Handlers.StandardHandlers do
  @moduledoc """
  ...
  def register_handlers do
    handlers = [
      ...
      {HydepwnsLiveview.Events.Handlers.LoggingHandler, []}
    ]
  ...
  * `{:error, reason}` - The handler could not be registered
  ...
  def register_handler(handler, opts \\ []) do
    ...
    if Code.ensure_loaded?(handler) do
      case HydepwnsLiveview.Events.Handlers.HandlerSupervisor.start_handler(handler, opts) do
        {:ok, _pid} ->
  ...
  def register_defaults do
    ...
    register_handler(HydepwnsLiveview.Events.Handlers.LoggingHandler)
  end
end

# Define standard handlers

defmodule HydepwnsLiveview.Events.Handlers.LoggingHandler do
  @moduledoc """
