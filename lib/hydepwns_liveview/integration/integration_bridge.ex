defmodule HydepwnsLiveview.Integration.IntegrationBridge do
  @moduledoc """
  ...
  * `{:error, reason}` - Failed to register adapter
  ...
  def register_adapter(name, adapter_module, config \\ %{}) do
    ...
    with {:module, _} <- Code.ensure_loaded(adapter_module),
      ...
      adapters = Application.get_env(:hydepwns_liveview, :integration_adapters, %{})
  ...
  * Map of adapter IDs to adapter info
  ...
  def list_adapters do
    Application.get_env(:hydepwns_liveview, :integration_adapters, %{})
  end
