defmodule HydepwnsLiveview.Resources.FolderResource do
  ...
  def validate(attrs) when is_map(attrs) do
    ...
    errors = if is_nil(attrs["content"]) or attrs["content"] == "", do: [{:content, "Content can't be blank"} | errors], else: errors
    errors = if attrs["parent_id"] && !is_binary(attrs["parent_id"]), do: [{:parent_id, "Parent ID must be a string or nil"} | errors], else: errors
