defmodule HydepwnsLiveview.Utils.SocketValidationHelper do
  @moduledoc """
  ...
  def broadcast_error(view_module, error_type, message, details \\ %{}) do
    if Mix.env() == :dev do
      ...
      PubSub.broadcast(HydepwnsLiveview.PubSub, @pubsub_topic, {:validation_error, error_data})
  ...
  def clear_errors do
    if Mix.env() == :dev do
      PubSub.broadcast(HydepwnsLiveview.PubSub, @pubsub_topic, :clear_errors)
    end
  end
  ...
  def subscribe do
    if Mix.env() == :dev do
      PubSub.subscribe(HydepwnsLiveview.PubSub, @pubsub_topic)
    end
  end
end
