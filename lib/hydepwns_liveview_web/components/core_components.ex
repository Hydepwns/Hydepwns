defmodule HydepwnsLiveviewWeb.CoreComponents do
  @moduledoc """
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        <thead class="text-sm text-left leading-6 text-zinc-500">
          <tr>
            <th :for={_col <- @_col} class="p-0 pb-4 pr-6 font-normal">{_col[:label]}</th>
        ...
        <tbody id={@id} phx-update={match?(%Phoenix.LiveView.LiveStream{}, @rows) && "stream"} class="relative divide-y divide-zinc-100 border-t border-zinc-200 text-sm leading-6 text-zinc-700">
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50">
            <td :for={{_col, i} <- Enum.with_index(@_col)} phx-click={@row_click && @row_click.(row)} class={["relative p-0", @row_click && "hover:cursor-pointer"]}>
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                ...
                <span :for={_action <- @_action} class="relative ml-4 font-semibold leading-6 text-zinc-900 hover:text-zinc-700">
  ...
  def list(assigns) do
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-zinc-100">
        <div :for={_item <- @_item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
