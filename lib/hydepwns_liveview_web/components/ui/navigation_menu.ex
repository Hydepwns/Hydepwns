defmodule HydepwnsLiveviewWeb.Components.UI.NavigationMenu do
  @moduledoc """
        ...
        <ul class="nav-list">
          <%= for item <- @item do %>
            <li class="nav-item">
              <a
                href={item.href}
                class={"nav-link #{if @current_path == item.href, do: "active"}"}
                aria-current={if @current_path == item.href, do: "page", else: "false"}
              >
                <%= render_slot(item) %>
              </a>
            </li>
