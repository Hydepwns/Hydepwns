defmodule HydepwnsLiveviewWeb.Components.UI.TableComponents do
  @moduledoc """
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50">
            <td :for={{_col, i} <- Enum.with_index(@col)} phx-click={@row_click && @row_click.(row)} class={["relative p-0", @row_click && "hover:cursor-pointer"]}>
              <div class="block py-4 pr-6">
                ...
                <span :for={_col <- @col} class={["relative", i == 0 && "font-semibold text-zinc-900"]}>
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                ...
                <span :for={_action <- @action} class="relative ml-4 font-semibold leading-6 text-zinc-900 hover:text-zinc-700">
