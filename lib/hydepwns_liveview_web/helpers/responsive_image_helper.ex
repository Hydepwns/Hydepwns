defmodule HydepwnsLiveviewWeb.ResponsiveImageHelper do
  @moduledoc """
  ...
  * `:widths` - List of widths to include (defaults to all available widths)
  ...
  def responsive_image_tag(image_path, opts \\ []) do
    ...
    if responsive_dir_exists do
      ...
      PhoenixHTMLHelpers.Tag.content_tag(:picture, [
        ...
        PhoenixHTMLHelpers.Tag.tag(:img,
          Keyword.merge(
      ...
      HydepwnsLiveviewWeb.ImageHelper.optimized_image_tag(image_path, opts)
    end
  end
  ...
  def placeholder_image_url(image_path) do
    ...
    if responsive_directory_exists?(filename) do
      ...
      HydepwnsLiveviewWeb.Endpoint.static_path(placeholder_path)
    else
  ...
  defp responsive_directory_exists?(filename) do
    responsive_dir = Path.join([
      Application.app_dir(:hydepwns_liveview, "priv/static/images/responsive"),
      filename
    ])
