defmodule HydepwnsLiveviewWeb.Examples.NestedValidationExampleLive do
  @moduledoc """
  ...
  def handle_event("introduce_errors", _params, socket) do
    ...
    [post | rest_posts] = posts
    # Empty title
    post = Map.put(post, :title, "")

    # Update the user with the invalid data
    user =
      user
      |> Map.put(:team, team)
    ...
    <div class="nested-validation-example p-6">
      ...
      <%= if @validation_status == "error" || @validation_status == "checkpoint" do %>
        <div class="validation-errors-section mt-6">
          ...
          <.live_component module={HydepwnsLiveviewWeb.Components.ValidationErrorsViewer} id="error-viewer" resource={@user} errors={@validation_errors} view_mode={@error_view_mode} include_resource_ids={@include_resource_ids} show_nested={true} max_depth={4} />
        </div>
      <% end %>
    </div>
    """
