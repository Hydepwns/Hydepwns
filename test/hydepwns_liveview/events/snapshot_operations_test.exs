defmodule HydepwnsLiveview.Events.SnapshotOperationsTest do
  ...
  describe "save_snapshot/4" do
    test "saves a valid snapshot" do
      ...
      assert {:ok, snapshot} = SnapshotOperations.save_snapshot("test_resource", "123", state, metadata)
    ...
    test "rejects invalid parameters" do
      ...
      assert {:error, :invalid_parameters} = SnapshotOperations.save_snapshot("test", "", %{}, %{})
      assert {:error, :invalid_parameters} = SnapshotOperations.save_snapshot("test", "123", "not_a_map", %{})
      assert {:error, :invalid_parameters} = SnapshotOperations.save_snapshot("test", "123", %{}, "not_a_map")
    end
  end
  ...
  describe "get_latest_snapshot/2" do
    setup do
      ...
      {:ok, snapshot1} = SnapshotOperations.save_snapshot("test_resource", "123", %{value: 1}, %{version: 1})
      {:ok, snapshot2} = SnapshotOperations.save_snapshot("test_resource", "123", %{value: 2}, %{version: 2})
      {:ok, snapshot3} = SnapshotOperations.save_snapshot("test_resource", "123", %{value: 3}, %{version: 3})
    ...
    test "returns not found for non-existent resource" do
      assert {:error, :not_found} = SnapshotOperations.get_latest_snapshot("test_resource", "nonexistent")
    end
  ...
  describe "count_events_since_last_snapshot/2" do
    setup do
      ...
      {:ok, snapshot} = SnapshotOperations.save_snapshot("test_resource", "123", %{value: 1}, %{event_id: "event1"})
    ...
    test "counts events since last snapshot" do
      assert {:ok, count} = SnapshotOperations.count_events_since_last_snapshot("test_resource", "123")
      assert count == 2
    end

    test "counts all events when no snapshot exists" do
      assert {:ok, count} = SnapshotOperations.count_events_since_last_snapshot("test_resource", "nonexistent")
      assert count == 2
    end

    test "rejects invalid parameters" do
      assert {:error, :invalid_parameters} = SnapshotOperations.count_events_since_last_snapshot("", "123")
      assert {:error, :invalid_parameters} = SnapshotOperations.count_events_since_last_snapshot("test", "")
    end
  end
  ...
  describe "save_versioned_state/4" do
    test "saves a valid versioned state" do
      ...
      assert {:ok, versioned_state} = SnapshotOperations.save_versioned_state("test_resource", "123", state, opts)
    ...
    test "rejects invalid parameters" do
      assert {:error, :invalid_parameters} = SnapshotOperations.save_versioned_state("", "123", %{}, [label: "test"])
      assert {:error, :invalid_parameters} = SnapshotOperations.save_versioned_state("test", "", %{}, [label: "test"])
      assert {:error, :invalid_parameters} = SnapshotOperations.save_versioned_state("test", "123", "not_a_map", [label: "test"])
      assert {:error, :invalid_parameters} = SnapshotOperations.save_versioned_state("test", "123", %{}, "not_a_list")
    end
  end
