defmodule HydepwnsLiveviewWeb.Features.RelationshipManagementWorkflowTest do
  ...
  setup %{session: session} do
    ...
    user = ResourceFixtures.create_user(%{name: "Test User"})
    team = ResourceFixtures.create_team(%{name: "Test Team"})
    post = ResourceFixtures.create_post(%{title: "Test Post", user_id: user.id})
    
    # Assign user to team
    {:ok, _} = HydepwnsLiveview.Resources.UserResource.update(user, %{team_id: team.id})
  ...
  describe "creating relationships" do
    ...
    test "user can add a relationship to existing resource", %{session: session, user: user, post: post} do
  ...
  describe "relationship validation" do
    test "system prevents invalid relationships", %{session: session} do
      ...
      |> fill_in(text_field("post[user_id]"), with: "999999") # Non-existent ID
      |> click(button("Create Post"))
  ...
  describe "advanced relationship features" do
    ...
    test "user can work with polymorphic relationships", %{session: session, user: user, post: post} do
