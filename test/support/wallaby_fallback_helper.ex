defmodule HydepwnsLiveviewWeb.TestHelpers.WallabyFallback do
  @moduledoc """
  ...
  """
  def find_elements_with_fallback(session, css_selector, regex_pattern, expected_values, _timeout \\ 1000) do
    # Try Wallaby first
    case Wallaby.Browser.all(session, css(css_selector)) do
      elements when elements != [] ->
        ...
        for value <- expected_values do
          ...
          end), "Expected to find element containing '#{value}'"
        end
        elements

      [] ->
        # Wallaby failed, try regex fallback
        :timer.sleep(100)  # Small delay for DOM updates
    ...
    case Wallaby.Browser.all(session, css(css_selector)) do
      elements when elements != [] ->
        ...
        for {attr, value} <- expected_attributes do
          ...
          end), "Expected to find element with #{attr}='#{value}'"
        end
      ...
      [] ->
        ...
        case Regex.scan(regex_pattern, html) do
          matches when matches != [] ->
            ...
            for {attr, value} <- expected_attributes do
              assert html =~ "#{attr}=\"#{value}\"", "Expected to find #{attr}='#{value}' in page source"
            end
  ...
  def assert_text_with_fallback(session, css_selector, regex_pattern, expected_text) do
    ...
    case Wallaby.Browser.all(session, css(css_selector)) do
      elements when elements != [] ->
        ...
        end), "Expected to find text '#{expected_text}' in elements with selector '#{css_selector}'"
      [] ->
        # Wallaby failed, try regex fallback
        html = Wallaby.Browser.page_source(session)
        matches = Regex.scan(regex_pattern, html)
        found_texts = Enum.map(matches, fn match ->
        ...
        assert Enum.any?(found_texts, &(&1 =~ expected_text)), "Expected text '#{expected_text}' not found in elements with selector '#{css_selector}'. Found texts: #{inspect(found_texts)}"
    end
  end
  ...
  def assert_count_with_fallback(session, css_selector, regex_pattern, expected_count) do
    ...
    case Wallaby.Browser.all(session, css(css_selector)) do
      elements when elements != [] ->
        ...
        assert actual_count == expected_count, "Expected #{expected_count} elements, got #{actual_count} for selector '#{css_selector}'"
      [] ->
        # Wallaby failed, try regex fallback
        html = Wallaby.Browser.page_source(session)
        matches = Regex.scan(regex_pattern, html)
        actual_count = length(matches)
        assert actual_count == expected_count, "Expected #{expected_count} elements with selector '#{css_selector}', got #{actual_count} from regex"
    end
  end
end 